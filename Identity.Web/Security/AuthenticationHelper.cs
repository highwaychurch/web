...
namespace Highway.Identity.Web.Security
{
    public class AuthenticationHelper
    {
        ...
        readonly IRelyingPartyRepository _relyingPartyRepository;
        ...
        public bool TryGetPrincipalFromOAuth2Request(HttpRequestBase request, ResourceOwnerCredentialRequest tokenRequest, out IClaimsPrincipal principal)
        {
            ...
            if (_userRepository.ValidateUser(tokenRequest.UserName ?? "", tokenRequest.Password ?? ""))
            {
        ...
        public bool TryGetPrincipalFromWrapRequest(HttpRequestBase request, out IClaimsPrincipal principal)
        {
            ...
            {
                if (_userRepository.ValidateUser(userNameId.Name, userNameId.Password))
                {
        ...
        public bool TryGetClientCertificatePrinciaplFromRequest(HttpRequestBase request, out IClaimsPrincipal principal)
        {
            ...
            {
                string userName;
                if (_userRepository.ValidateUser(clientCertificate, out userName))
                {
        ...
        public bool TryGetBasicAuthenticationPrincipalFromRequest(HttpRequestBase request, out IClaimsPrincipal principal)
        {
            ...
            {
                if (_userRepository.ValidateUser(identity.Name, identity.Password))
                {
        ...
        public RelyingPartyModel GetRelyingPartyDetails(string realm)
        {
            RelyingPartyModel rp = null;

            if (_relyingPartyRepository.TryGet(realm, out rp))
            {
                return rp;
            }
        ...
        public RelyingPartyModel GetRelyingPartyDetailsFromReturnUrl(string returnUrl)
        {
